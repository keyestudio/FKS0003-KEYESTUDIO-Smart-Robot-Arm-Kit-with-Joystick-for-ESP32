...
US: [https://www.amazon.com/keyestudio](https://www.amazon.com/keyestudio)

Canada: [https://www.amazon.ca/keyestudio](https://www.amazon.ca/keyestudio)

Japan: [https://www.amazon.co.jp/keyestudio](https://www.amazon.co.jp/keyestudio)  

UK: [https://www.amazon.co.uk/keyestudio](https://www.amazon.co.uk/keyestudio)

Germany: [https://www.amazon.de/keyestudio](https://www.amazon.de/keyestudio)

France: [https://www.amazon.fr/keyestudio](https://www.amazon.fr/keyestudio)

Italy: [https://www.amazon.it/keyestudio](https://www.amazon.it/keyestudio)

Spain: [https://www.amazon.es/keyestudio](https://www.amazon.es/keyestudio)
...
|  13  |        ![img](./media/wps22.jpg)         |         3 pin M-F 20CM DuPont wire (black-red-white)         |  1   |
...
|  18  |        ![img](./media/wps27.jpg)         |                       2 bearing covers                       |  1   |
...
Based on ESP32 development board, we utilize various servos to build up a programmable robot arm, which is able to wave, grasp and lift objects. It can be controlled by WiFi and Bluetooth to communicate with other devices. 

This is a brilliant maker project for beginners to learn the using methods of ESP32 board and servo. Besides, for developers, sophisticated controlling can be achieved by more advanced programming.
...
Battery holder: 6-slot AA battery holder  / 2-slot 18650 battery holder 
...
## 7. Arduino IDE Basic Tutorial

Click to check details: [Arduino IDE Basic Tutorial](http://getting-started-with-arduino.readthedocs.io/), including Arduino IDE download and using methods, as well as how to install the driver of the development board. 

<p style="color:red;">NOTE: ESP32 development board of version 2.0.12 is recommended, since our tutorials are based on this version. Incompatibilities may exist if other versions are used.</p>

For Windows, there is an easier way to install the ESP32, click the link to download `esp32_package_2.0.12_arduinome.exe`. Click to enable the automatic installation. After that, open Arduino IDE and you will see ESP32: 

`esp32_package_2.0.12_arduinome.exe` download link: [https://fs.keyestudio.com/ESP32](https://fs.keyestudio.com/ESP32)

![7-2](./media/7-2.png)

<p style="font-size:25px;color:red;">For beginners, please learn the Arduino IDE basic tutorials first. They will help you to avoid a lot of mistakes.</p>
...
#### 8.1.1 Mount Battery Holder

##### 8.1.1.1 Six-slot AA Battery Holder
...
##### 8.1.1.2 Two-slot 18650 Battery Holder
...
#### 8.1.2 Demount Battery Holder

##### 8.1.2.1 Six-slot AA Battery Holder
...
##### 8.1.2.2 Two-slot 18650 Battery Holder
...
### 8.2 Expansion Board and ESP32 Development Board
...
### 8.4 Base for Arm/Bearing and Base Servo
...
### 8.5 Joint of Arm and Base Part
...
### 8.6 Right-side Upper Arm and Its Servo
...
Download codes: [Codes](./codes.zip)

Download and unzip the file. Open the corresponding code file.  Code file for Chapter 8.6 is named 8-6.

![8-6-6](./media/8-6-6.png)
...
### 8.8 Left-side Forearm/Upper Arm and Forearm Servo
...
Download and unzip the file. Open the corresponding code file.  Code file for Chapter 8.8 is named 8-8.

![8-8-8](./media/8-8-8.png)
...
### 8.9 Base for Claw and Its Servo
...
Download and unzip the file. Open the corresponding code file.  Code file for Chapter 8.10 is named 8-10.

![8-10-8](./media/8-10-8.png)
...
Download and unzip the file. Open the corresponding code file.  Code file for Chapter 8.12 is named 8-12.

![8-12-10](./media/8-12-10.png)
...
<p style="color:red;">ATTENTION: Connect servo to pin G(GND), V(VCC), IO16: brown wire for Gnd(G), red for 5v(V), orange for IO16. External power supply is required, because the current of the development board is far from meeting the relatively large current requirements for driving the servo.</p>
...
Download and unzip the file. Open the corresponding code file.  Code file for Chapter 9.1.5is named 9-1-5.

![9-1-5](./media/9-1-5.png)
...
After uploading the code, the servo rotates from 0° to 90° and delays for 1s, and then rotates to 180° and also delays for 1s. At last, it back to 0° to repeat these actions.
...
In this extension experiment, we program to control the servo to slowly rotate from 0° to 180° and then slowly back to 0° degree by degree.
...
![9-2-3-2](./media/9-2-3-2.png)



#### 9.2.4 Wiring Diagram

![9-2-4](./media/9-2-4.png)
...
void setup() {
  ...
  pinMode(buttonPin, INPUT);  //set pin buttonPin to input.引脚为输入模式
}
...
After uploading code, click ![9-2-6](./media/9-2-6-1.png) on Arduino IDE to open the serial monitor, and click ![9-2-6](./media/9-2-6-2.png) to set baud rate to 9600. The joystick values will be displayed on the monitor.

<p style="color:red">NOTE: the joystick value will not reach 4095 without an external power supply.</p>

![9-2-6-3](./media/9-2-6-3.png)

#### 9.2.7 Read the remote lever value of the handle

Connect two remote rod sensors to the ESP32 servo drive board, then read the values and print them through the serial port.

##### 9.2.7.1Wiring Diagram
...
Download and unzip the file. Open the corresponding code file.  Code file for Chapter 9.2.7.2is named 9-2-7-2.

![9-2-7-2](./media/9-2-7-2.png)

```c
/*
  Keyestudio ESP32 Robot Arm
  9-2-7-2 tutorial code
  Function: 
  http://www.keyestudio.com
*/
...
After uploading the code, open the serial port monitor and set the serial port baud rate to 9600, so that you can see the printed remote control value. If you feel that the serial port printing speed is too fast, you can increase the value in the parentheses of the delay ().

![9-2-7-3](./media/9-2-7-3.png)
...
![9-3-4](./media/9-3-4.png)

#### 9.3.5 Add a Record Function
...
After uploading the code, we control the arm to perform actions and then press the right joystick to save the angles of servo. When you press the left joystick, these saved actions will be played back.
...
In this experiment, we will control the arm through WiFi. We just need to connect a mobile phone/pad and ESP32 board to one wifi frequency of 2.4GHz (ESP32 only connect to 2.4GHz frequency wifi). After uploading code, we can control the arm by wifi.
...
Download and unzip the file. Open the corresponding code file.  Code file for Chapter 9.4.2 is named 9-4-2-1. (because there are two codes in this section, add -1 and -2 after 9-4-2 to distinguish)

![9-4-2-6](./media/9-4-2-6.png)

Ensure the ESP32 correctly connects to Wi-Fi first. You may refer to the following code:

```c++
/*
  Keyestudio ESP32 Robot Arm
  9-4-2-1 tutorial code
...
In this code, please modify `ssid` and `password` into your Wi-Fi name and passwords respectively.
...
After uploading the code, serial monitor prints the Wi-Fi connecting state and IP address.
...
Download and unzip the file. Open the corresponding code file.  Code file for Chapter 9.4.2 is named 9-4-2-2. (because there are two codes in this section, add -1 and -2 after 9-4-2 to distinguish)

![9-4-2-7](./media/9-4-2-7.png)

Once Wi-Fi is connected, ESP32's Web server will serve up web pages. In the following example, we will create a simple web page that says "Hello, World!" :
...
**After uploading code, the serial monitor displays ESP32 IP address. Search “http://[ESP32 IP address]” and visit it in browser, and you will see the web page showing “Hello, World!”.**

**When PC, mobile device and ESP32 board connect to the same network, you can visit that page on two devices simultaneously. Here will be your IP address of ESP32.**
...
##### 9.4.3.3 Web Page Control Servo

After uploading code, open Arduino IDE and set baud rate to 9600. After the wifi is connected, the serial monitor prints the IP address of ESP32:
