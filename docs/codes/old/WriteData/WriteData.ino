...
void loop() {
  ...
  // int left_joy_x = map(left_X_data, 0, 4096, 0, 180);
  // int left_joy_y = map(left_Y_data,0,4096,0,90);
  // int right_joy_y = map(right_Y_data,0,4096,0,90);
  // base.write(left_joy_x);
  // smoothRotate(left_joy_x);
  // boom.write(left_joy_y);
  // forearm.write(right_joy_y);
  attitude();
  baseControl();
  armControl();
  forearmControl();
  ...
  // Serial.print(right_Y_data);
  // Serial.print("    right_X:");
  // Serial.println(right_X_data);
  // delay(1000);
}
